...
AccountManagement.login = function(email, password, callback) {
  ...
  if(!AccountManagement.looksLikeAnEmail(email)) {
    setImmediate(function() {
      callback(new FieldError(FieldError.TYPES.WRONG_FORMAT, 'email', 'john-doe@john-doe-enterprises.tld'), undefined)
    })
    return
  }
...
AccountManagement.filterAccount = function(account) {
  var filtered = {
    email: account.email,
